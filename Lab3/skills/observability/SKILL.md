---
name: observability
description: Performs ADLC Step 17 Observability, Tracing & Logging Verification of Deployed Target Agent.
---
...
3. Verify that the Target Agent has been deployed (`deployment` skill / Step 10) and that `deployment_metadata.json` exists in `lab3_cymbal_navigation_planner/` or container deployment variables are known.
...
5. Capture the raw inspection command output and telemetry status checks to `artifacts/docs/step17_observability_log.txt`.
6. Capture the verified configuration state, active tiers, privacy settings, and overall status into `artifacts/docs/step17_observability_report.md`.

## Output
Produce `artifacts/docs/step17_observability_log.txt` with raw stdout/stderr, and `artifacts/docs/step17_observability_report.md` summarizing the observability verification.
